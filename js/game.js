...
function el(tag, attrs = {}, children = []) {
  ...
  for (const [k, v] of Object.entries(attrs)) {
    ...
    else if (k === "disabled") { if (v) e.setAttribute("disabled", ""); } // boolean HTML attr: presence = true, so a falsy value must not be set at all
...
function renderTabs() {
  ...
  nav.innerHTML = "";
  if (!STATE || !STATE.player || STATE.phase === "creation" || STATE.phase.includes("select") || STATE.phase.includes("prep") || STATE.phase === "draft-results" || STATE.phase === "contract-decision" || STATE.phase.includes("season") && STATE.phase !== "season") {
